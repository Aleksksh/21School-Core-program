...
START_TEST(fmod_tests) {
  ...
  y = 214124.123;
  ck_assert_ldouble_nan(s21_fmod(x, y));
  ck_assert_ldouble_nan(s21_fmod(214124.123, S21_NAN));
  ck_assert_ldouble_nan(s21_fmod(S21_NAN, 214124.123));
}
END_TEST
...
START_TEST(pow_tests) {
  ...
  ck_assert_ldouble_infinite(s21_pow(-S21_INF, 1.0));
...
START_TEST(sqrt_tests) {
  ...
  ck_assert_int_eq(sqrt(-4), s21_sqrt(-4));
